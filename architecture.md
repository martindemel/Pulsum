...
- The orchestrator enforces a three-wall guardrail—safety classification, on-topic gating, and retrieval coverage—before escalating to the GPT-5 cloud pathway or falling back to on-device generation. A dedicated `TopicSignalResolver` now maps intent topics to the real `z_*`/`subj_*` snapshot keys and the data-dominant fallback selects the max-|z| signal deterministically; Gate 6 adds an embedding-availability gate so coverage redirects on-device only when all on-device generators fail. (Packages/PulsumAgents/Sources/PulsumAgents/AgentOrchestrator.swift:221-520; Packages/PulsumServices/Sources/PulsumServices/LLMGateway.swift:136-270)
- Health metrics and journals flow through a `DataAgent` actor that merges HealthKit streams, subjective sliders, and sentiment embeddings to drive a wellbeing score and metric breakdown surfaced in UI dashboards. Gate 6 normalizes/clamps features (including sentiment), seeds recovery-positive weights, persists the StateEstimator on disk, and surfaces explicit wellbeing states (loading/ready/no-data/denied/error) in UI cards. (Packages/PulsumAgents/Sources/PulsumAgents/DataAgent.swift:46-216; Packages/PulsumAgents/Sources/PulsumAgents/EstimatorStateStore.swift:1-76; Packages/PulsumUI/Sources/PulsumUI/PulsumRootView.swift:5-274)
- Update (2025-10-23): Voice journaling now uses `beginVoiceJournal` → `speechStream` (async segments) → `finishVoiceJournal(transcript:)`; legacy single-call `recordVoiceJournal` flow is obsolete. (Packages/PulsumAgents/Sources/PulsumAgents/SentimentAgent.swift)
- Update (2025-10-23): Diagnostics notifications are handled via `DiagnosticsLogger`/`DebugLogBuffer`; the previously referenced `DiagnosticsNotifications.swift` file was removed.
- Clarification: Diagnostics persistence/signposts are controlled via `DiagnosticsConfigStore` and surfaced in Settings; DebugLogBuffer serves debug-only logs and should not be relied upon for production telemetry.
...
- `Packages/PulsumData/` – Core Data stack, vector index implementation, and library importer that seeds recommendation content; the canonical `Pulsum.xcdatamodeld` now lives under `Sources/PulsumData/Resources/` so both the app bundle and package tests load the identical model via `Bundle.module`. The library importer now defers indexing when on-device embeddings are unavailable while still ingesting Core Data entities, retrying once embeddings are ready without blocking startup. (Packages/PulsumData/Package.swift:5-36; Packages/PulsumData/Sources/PulsumData/DataStack.swift:18-187; Packages/PulsumData/Sources/PulsumData/LibraryImporter.swift:41-203)
...
## 7. Modules & Layers
- **UI layer (PulsumUI)**: provides tab navigation, coach chat, pulse journal, settings, insights dashboards, and shared styling. View models encapsulate orchestrator calls and UI-specific state; `CoachViewModel` now updates wellbeing readiness from a snapshot-only call, tracks `WellbeingSnapshotKind` (none/placeholder/real) to render a warm-up card instead of a misleading no-data warning, and triggers recommendation refreshes on a debounced/coalesced path that never gates the score UI. (Packages/PulsumUI/Sources/PulsumUI/PulsumRootView.swift:84-274; Packages/PulsumUI/Sources/PulsumUI/CoachViewModel.swift:1-320; Packages/PulsumUI/Sources/PulsumUI/PulseViewModel.swift:8-189; Packages/PulsumUI/Sources/PulsumUI/SettingsViewModel.swift:9-205; Packages/PulsumUI/Sources/PulsumUI/PulsumDesignSystem.swift:6-172)
- **Wellbeing readiness path**: `AgentOrchestrator.wellbeingSnapshotState` returns a snapshot-only response (no recommendations awaited), `CoachViewModel` applies it immediately, and recommendations refresh on an async, coalesced path. `WellbeingSnapshotKind` semantics are: `none` = no snapshot yet, `placeholder` = warm-start placeholder published during initial ingestion, `real` = fully computed snapshot. Placeholder snapshots render a calm “Warming up…” card so first-run users aren’t shown an “insufficient samples” warning while HealthKit syncs. (Packages/PulsumAgents/Sources/PulsumAgents/AgentOrchestrator.swift:450-620; Packages/PulsumUI/Sources/PulsumUI/CoachViewModel.swift:1-320; Packages/PulsumUI/Sources/PulsumUI/PulsumRootView.swift:84-274; Packages/PulsumUI/Sources/PulsumUI/SettingsView.swift:610-980)
- **Agents layer (PulsumAgents)**: `AgentOrchestrator` composes agents for data, sentiment, coaching, safety, and celebrations, exposing async API to the UI. Gate 6 adds an embedding availability gate before coverage so chats route on-device when both AFM and CoreML embeddings are down. (Packages/PulsumAgents/Sources/PulsumAgents/AgentOrchestrator.swift:65-404)
- `DataAgent` actor integrates HealthKit (observer + anchored queries), subjective inputs, Core Data persistence, state estimation, and score breakdown assembly. Gate 6 now bootstraps a first score from the last 2 days, then schedules the 7-day warm-start + 30-day backfill in the background with per-type progress in `BackfillStateStore` so UI stays responsive; `.pulsumScoresUpdated` is debounced to avoid card flicker. (Packages/PulsumAgents/Sources/PulsumAgents/DataAgent.swift:46-220; Packages/PulsumAgents/Sources/PulsumAgents/DataAgent.swift:117-216)
  - `CoachAgent` handles recommendation retrieval from the vector index, ranks with `RecRanker`, and gifts context to `LLMGateway` or on-device responders. Keyword fallback now fetches a bounded, deterministic set and filters Transformable tags in memory (tags are persisted via NSSecureUnarchive BLOBs) to avoid Core Data predicate crashes when embeddings are unavailable. (Packages/PulsumAgents/Sources/PulsumAgents/CoachAgent.swift:11-200, 383-418; Packages/PulsumAgents/Sources/PulsumAgents/CoachAgent+Coverage.swift:49-107)
...
- Note: `ModernSpeechBackend` currently delegates to the legacy backend (stub behavior) until SpeechAnalyzer/SpeechTranscriber APIs are integrated. (Packages/PulsumServices/Sources/PulsumServices/SpeechService.swift:500-534)
...
- **ML utilities (PulsumML)**: On-device embeddings, sentiment, safety, and ranking.
  - `EmbeddingService` probes availability once, prefers AFM contextual embeddings when Apple Intelligence is ready, and falls back to the bundled CoreML model; zero vectors now throw and `generatorUnavailable` only fires when both tiers fail. (Packages/PulsumML/Sources/PulsumML/Embedding/EmbeddingService.swift:4-74)
  - `EmbeddingTopicGateProvider` scores on-topic vs. out-of-domain using prototype embeddings and confidence margins, failing closed when embeddings are unavailable. (Packages/PulsumML/Sources/PulsumML/TopicGate/EmbeddingTopicGateProvider.swift:4-168)
  ...
  - `SafetyLocal`, `StateEstimator`, and `RecRanker` provide local guardrails, wellbeing estimation, and recommendation ranking heuristics. StateEstimator now seeds recovery-positive weights (including sentiment) and persists state; RecRanker updates from acceptance/dismiss feedback. (Packages/PulsumML/Sources/PulsumML/SafetyLocal.swift:31-172; Packages/PulsumML/Sources/PulsumML/StateEstimator.swift:3-120; Packages/PulsumML/Sources/PulsumML/RecRanker.swift:3-170)
...
- `DataAgent` constructs `FeatureVectorSnapshot` aggregates, persists subjective sliders, and materializes `ScoreBreakdown` metrics with baseline comparisons and imputation notes. Gate 6 adds WellbeingModeling normalization/clamping (including sentiment), recovery-positive seeds, and persisted StateEstimator state so personalization survives relaunches. (Packages/PulsumAgents/Sources/PulsumAgents/DataAgent.swift:8-44; Packages/PulsumAgents/Sources/PulsumAgents/DataAgent.swift:117-216; Packages/PulsumAgents/Sources/PulsumAgents/EstimatorStateStore.swift:1-76)
...
- Structured diagnostics now use an on-device `DiagnosticsLogger` (session + trace IDs, spans, stall monitors) that mirrors to `DebugLogBuffer`, optional OSLog, and (currently default-on) file persistence/signposts to aid stall investigation. Users can toggle diagnostics/persistence and export a single report (header + snapshot + log tail) from Settings; no PHI, transcripts, or raw HealthKit samples are logged or exported. (Packages/PulsumTypes/Sources/PulsumTypes/DiagnosticsLogger.swift; Packages/PulsumUI/Sources/PulsumUI/SettingsView.swift)
...
- Unit tests must not trigger heavyweight startup. The legacy skip-startup flag has been removed; in DEBUG builds `AppViewModel.start()` short-circuits only when XCTest sets either `XCTestConfigurationFilePath` or `XCTestBundlePath` (presence is enough, even if empty), and the bypass is compiled out of Release so normal runs always execute startup. (Packages/PulsumUI/Sources/PulsumUI/AppViewModel.swift)
- UI test targets exercise permissions, journaling, and coach/settings flows via deterministic stubs, with env flags injected from `PulsumUITestCase.launchPulsum()` so shared schemes stay clean for Run. (PulsumUITests/FirstRunPermissionsUITests.swift, PulsumUITests/JournalFlowUITests.swift, PulsumUITests/SettingsAndCoachUITests.swift)
...
- Shared schemes must not inject test-only startup-bypass environment variables; UI tests set launch arguments/environments in code (`launchEnvironment`/`launchArguments`), not in the scheme.
...
- Configuration uses per-developer `.xcconfig` files; the checked-in template contains a placeholder `OPENAI_API_KEY` value and `Config.xcconfig` is empty, so no real secrets are committed. Runtime keys still come from Keychain or `PULSUM_COACH_API_KEY`. (Config.xcconfig:1-5; Config.xcconfig.template:1-5; Packages/PulsumServices/Sources/PulsumServices/LLMGateway.swift:279-462)
