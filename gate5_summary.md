...
- Refactored `LibraryImporter.ingestIfNeeded()` so file reads/decoding happen **before** `context.perform`; the Core Data block now only upserts managed objects and returns DTO payloads. Vector index upserts run *after* the Core Data work, keeping the queue non‑blocking. (`Packages/PulsumData/Sources/PulsumData/LibraryImporter.swift:40-210`)
...
## Tests & Harness
- **PulsumData:** `Gate5_VectorIndexConcurrencyTests` stress concurrent upsert/search/remove across multiple shards; `Gate5_VectorIndexFileHandleTests` inject failing handles to ensure `close()` errors propagate; `Gate5_VectorIndexManagerActorTests` verify actor usage from background contexts; `Gate5_LibraryImporterPerfTests` time concurrent reads during imports. (`Packages/PulsumData/Tests/PulsumDataTests/…`)
