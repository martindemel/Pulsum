...
- Refactored `LibraryImporter.ingestIfNeeded()` so file reads/decoding happen **before** `context.perform`; the Core Data block now only upserts managed objects and returns DTO payloads. Vector index upserts run *after* the Core Data work and **only then** persist `LibraryIngest.checksum`, so transient indexing failures retry safely. (`Packages/PulsumData/Sources/PulsumData/LibraryImporter.swift:40-210`)
